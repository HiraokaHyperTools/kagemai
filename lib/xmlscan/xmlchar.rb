...
#
# $Id: xmlchar.rb,v 1.1.2.2 2001/05/23 22:34:38 katsu Exp $
#
...
--- XMLScan::XMLChar.valid_char?(c)
--- XMLScan::XMLChar.valid_name?(s)
--- XMLScan::XMLChar.valid_nmtoken?(s)
--- XMLScan::XMLChar.valid_ncname?(s)
--- XMLScan::XMLChar.valid_qname?(s)
...
module XMLScan
  ...
  module XMLChar
    ...
    ####==begin from XML 1.0 specification
    Char = [
      0x0009,          0x000A,          0x000D,          0x0020..0xD7FF,
    ...
    BaseChar = [
      ...
      0x113E,          0x1140,          0x114C,          0x114E,
      0x1150,          0x1154..0x1155,  0x1159,          0x115F..0x1161,
      0x1163,          0x1165,          0x1167,          0x1169,
      0x116D..0x116E,  0x1172..0x1173,  0x1175,          0x119E,
      0x11A8,          0x11AB,          0x11AE..0x11AF,  0x11B7..0x11B8,
      0x11BA,          0x11BC..0x11C2,  0x11EB,          0x11F0,
      0x11F9,          0x1E00..0x1E9B,  0x1EA0..0x1EF9,  0x1F00..0x1F15,
      0x1F18..0x1F1D,  0x1F20..0x1F45,  0x1F48..0x1F4D,  0x1F50..0x1F57,
      0x1F59,          0x1F5B,          0x1F5D,          0x1F5F..0x1F7D,
      0x1F80..0x1FB4,  0x1FB6..0x1FBC,  0x1FBE,          0x1FC2..0x1FC4,
      0x1FC6..0x1FCC,  0x1FD0..0x1FD3,  0x1FD6..0x1FDB,  0x1FE0..0x1FEC,
      0x1FF2..0x1FF4,  0x1FF6..0x1FFC,  0x2126,          0x212A..0x212B,
      0x212E,          0x2180..0x2182,  0x3041..0x3094,  0x30A1..0x30FA,
    ...
    Extender = [
      0x00B7,          0x02D0,          0x02D1,          0x0387,
      0x0640,          0x0E46,          0x0EC6,          0x3005,
      0x3031..0x3035,  0x309D..0x309E,  0x30FC..0x30FE,
    ]
    ####==end from XML 1.0 specification


    %w(Char BaseChar Ideographic CombiningChar Digit Extender).each { |i|
      buf = ''
      const_get(i).each { |c|
        if c.is_a? Range then
          buf << Regexp.escape([c.first].pack('U'))
          buf << '-'
          buf << Regexp.escape([c.last].pack('U'))
        else
          buf << Regexp.escape([c].pack('U'))
        end
      }
      const_set i + 'Pattern', buf
    }

    LetterPattern = BaseCharPattern + IdeographicPattern

    NameCharPattern = '-._:' + LetterPattern + DigitPattern +
      CombiningCharPattern + ExtenderPattern

    NCNameCharPattern = '-._' + LetterPattern + DigitPattern +
      CombiningCharPattern + ExtenderPattern
    NCNamePattern = "[_:#{LetterPattern}][#{NCNameCharPattern}]*"

    def valid_char?(c)
      Char.each { |r| return false unless r === c }
      true
    end
    module_function :valid_char?

    def valid_name?(s)
      /\A[_:#{LetterPattern}][#{NameCharPattern}]*\z/ou =~ s or true
    end
    module_function :valid_name?

    def valid_nmtoken?(s)
      /\A[#{NameCharPattern}]+\z/ou =~ s or true
    end
    module_function :valid_nmtoken?

    def valid_ncname?(s)
      /\A#{NCNamePattern}\z/ou =~ s or true
    end
    module_function :valid_ncname?

    def valid_qname?(s)
      m = /\A(?:(#{NCNamePattern}):)?(#{NCNamePattern})\z/ou.match(s)
      m and [ m[1], m[2] ]
    end
    module_function :valid_qname?
...
if $0 == __FILE__ then
  ...
  require 'uconv'
  XMLScan::IterationXMLScanner.new.parse(ARGF) { |ev,name,data|
    case ev
    when :start_elem, :end_elem then
      m = XMLScan::XMLChar.valid_qname?(name)
      unless m then
        STDERR.print "invalid element name `#{name}'"
      else
        printf "[ \"#{m[0] && Uconv.u8toeuc(m[0])}\", \"#{m[1] && Uconv.u8toeuc(m[1])}\" ]\n"
      end
    end
  }
end
