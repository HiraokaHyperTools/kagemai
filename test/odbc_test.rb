...
if do_odbc_test then
  ...
  class TestODBCStore < Test::Unit::TestCase
    def setup()
    ...
    def test_load
      ...
      report.each_with_index do |message, i|
        ...
        assert_equal(@messages[i].hide?, message.hide?)
    ...
    def test_capital_project()
      project_id = 'ODBCStoreTestProject'
    ...
    def test_sql_time()
      ...
      fmt = '%Y-%m-%d %H:%M'
      assert_equal(now.strftime(fmt), @store.load_time(row['create_time']).strftime(fmt))
    end
